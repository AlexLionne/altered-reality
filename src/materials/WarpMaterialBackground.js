...
export default class extends ShaderMaterial {
    constructor() {
        ...
        `,
          ...
          float rand(vec3 p){ return fract(sin(dot(p,vec3(12.9898,78.233,45.5432)))*43758.5453); }
          float noise(vec2 p,float s){
            vec2 ip=floor(p), u=fract(p);
            u=u*u*(3.0-2.0*u);
            float a=rand(vec3(ip,s)), b=rand(vec3(ip+vec2(1.0,0.0),s));
            float c=rand(vec3(ip+vec2(0.0,1.0),s)), d=rand(vec3(ip+vec2(1.0,1.0),s));
          ...
          vec3 colormapColor(float x){
            ...
            int idx=int(floor(x/step)); float t=(x-float(idx)*step)/step;
          ...
          void main(){
            ...
            float dx=pattern(uv,uSeed), dy=pattern(uv+vec2(1.7,2.3),uSeed);
            ...
            if(uNumColors>0){
              ...
              baseCol=mix(vec4(0.),vec4(rgb,1.),a);
            }
            gl_FragColor=vec4(baseCol.rgb,uOpacity);
          }
        `,
            transparent: false,
